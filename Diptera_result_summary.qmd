---
title: DIPTERA STATS AND MODELS
...
diptera <- read_xlsx("Data/Breitenbach_community_data_17.12.2024.xlsx") |>     # read in excel spreadsheet
  ...
  filter(order %in% "Diptera") |>    # Keeps only EPT taxa
...
diptera_agg <- diptera_long |>
  group_by(sp_name,
           trap, 
           family) |>                                                      # Group by sp_name, family, and trap
  summarise(abundance = sum(abundance),
            .groups = "drop") |>                                           # Summarise with the sum of "abundance"
  arrange(trap) |>                                                         # Sort data
  filter(!trap %in% c("F", "Source", "D")) |>                              # Remove unwanted traps
...
write_xlsx(diptera_wide, "Taxonomy information/Diptera_taxa_for_analysis.xlsx")
...
chaoA       <- ChaoSpecies(diptera_wide$A ,"abundance", k = 2, conf = 0.95)        # trap A
chaoB       <- ChaoSpecies(diptera_wide$B ,"abundance", k = 2, conf = 0.95)        # trap B
chaoC       <- ChaoSpecies(diptera_wide$C ,"abundance", k = 2, conf = 0.95)        # trap C
chaoE       <- ChaoSpecies(diptera_wide$E ,"abundance", k = 2, conf = 0.95)        # trap E
chaoG       <- ChaoSpecies(diptera_wide$G ,"abundance", k = 2, conf = 0.95)        # trap G
chaoI       <- ChaoSpecies(diptera_wide$I ,"abundance", k = 2, conf = 0.95)        # trap I
chaoIII     <- ChaoSpecies(diptera_wide$III ,"abundance", k = 2, conf = 0.95)      # trap III
chaoOverall <- ChaoSpecies(diptera_wide$Overall ,"abundance", k = 2, conf = 0.95)      # Overall
```
...
attach(diptera_wide) # makes each column a separate, accessible vector
...
saveRDS(all_traps_data, "Outputs/ChoasDiversityProfilesDiptera.rds")
```
...
Diversity_profiles <- readRDS("Outputs/ChoasDiversityProfilesDiptera.rds")
```
...
# Create the plot
Diversity_profiles |> 
  filter(!(Trap %in% c("Trap F", "Trap Source"))) |> 
  mutate(
    Source = case_when(
  ...
  geom_hline(yintercept = seq(-50, y_max_rounded, by = 10), 
  ...
  scale_y_continuous(limits = c(-50, y_max_rounded), 
                     breaks = seq(0, y_max_rounded, by = 50)) +
...
ggsave("Plots/Diptera/Diversity_profiles_diptera.png", width = 10, height = 7, dpi = 300, bg = "white")
```
...
## Mblm trends in Diptera family abundances per trap

```{r}
diptera_summary <- diptera_long %>%
  filter(trap %in% c("A", "B", "C", "E", "G", "I", "III")) |> 
  group_by(trap, year, family) %>%
  summarise(total_abundance = sum(abundance), .groups = "drop")

trend_df <- diptera_summary %>%
  group_by(trap, family) %>%
  summarise(
    slope = {
      df <- pick(everything())
      if (n_distinct(df$year) > 1) {
        coef(mblm(total_abundance ~ year, data = df))[2]
      } else {
        NA_real_
      }
    },
...
trend_counts <- trend_df %>%
  group_by(family, trend) %>%
  summarise(n = n(), .groups = "drop")

overall_trend_df <- diptera_summary %>%
...
print(trend_counts, n = Inf)
print(trend_df, n = Inf)
print(overall_trend_df, n = Inf)

write_xlsx(trend_df, "Outputs/diptera_family_trends.xlsx")
```
...
diptera_long$ro.ab <- round(diptera_long$abundance, digits = 0) # Round abundance to whole numbers for rarefaction
...
for (i in unique(diptera_long$trap)) {                          # Loop through each unique trap in the dataset
  sub <- diptera_long[diptera_long$trap == i, ]                 # subset for current trap & reshape data
  sub_m <- sub |>
  ...
  S_PIE <- calc_PIE(sub_ta)                             # Effective number of common species
...
# Calculate maximum number of individuals
Max_diptera_individuals <- sum(diptera_wide_red$Overall, na.rm = T)
...
p <- p + 
  ...
  # scale_x_continuous(
  #   labels = scales::comma,
  #   breaks = function(x) seq(0, max(x), by = 50000)
  # ) +
  scale_x_continuous(
    labels = scales::comma,
...
# Save the plot
ggsave("Plots/Diptera/Rarefaction_diptera.png", p, width = 7, height = 7, dpi = 300, bg = "white")
```
...
p1 <- TD |>
  ...
  geom_smooth(method = "gam") +
  geom_smooth(method = "lm", col = "red") +
  labs(y = "Diptera species richness",
       x = "Time") +
  geom_jitter(shape = 19, size = 3.5, height = 0.5,
              width = 0.5, alpha = 0.5) +
  scale_x_continuous(limits = c(1965, 2005)) +
...
p1 <- TD |>
  ...
  geom_smooth(method = "gam") +
  geom_smooth(method = "lm", col = "red") +
  labs(y = "Diptera species turnover",
       x = "Time") +
  geom_jitter(shape = 19, size = 3.5, height = 0.5,
              width = 0.5, alpha = 0.5) +
  scale_x_continuous(limits = c(1965, 2005)) +
...
p2 <- TD |> 
  ggplot(aes(SppRich)) +
  geom_freqpoly(bins = 15) +
  labs(x = "Diptera species richness",
       y = "Frequency",
...
p3 <- TD |> 
  ggplot(aes(Turnover)) +
  geom_freqpoly(bins = 15) +
  labs(x = "Diptera species turnover",
       y = "Frequency",
...
saveRDS(TD, "Outputs/Diptera_TD.rds")
```
...
hist(TD$Abund, main = "Distribution of Diptera Abundance", breaks = 30, col = "steelblue")
hist(TD$SppRich, main = "Distribution of Diptera species richness", breaks = 30, col = "steelblue")
hist(TD$Turnover, main = "Distribution of Diptera Turnover", breaks = 30, col = "steelblue")
```

## Create filtered dataset for overall model

```{r}
#| label: Filter dataset for overall models
...
```{r}
#| label: Check non-linearity in abundance
#| fig-height: 8
#| fig-width: 12
...
mod_overall5 <- glmmTMB(Abund ~ ns(year.std, df = 4) + (1 | ftrap) + ar1(0 + factor(year.std) | ftrap),
                            family = nbinom2, 
                            data = TD_red)
...
for (i in seq_along(trap_models)) {
  plot(validation_results[[i]], title = names(trap_models)[i])
}
par(mfrow = c(1, 1))
```
...
tab_model(
  ...
  file = "Plots/Diptera/Diptera_abundance_overall_model.html"
)
...
tab_model(
  ...
  file = "Plots/Diptera/Diptera_abundance_trap_models.html"
)
```
...
ggplot() +
  ...
  labs(
    title = "Diptera Abundance Trends: Trap-Specific & Overall",
    x = "Standardized Year",
...
dip_abund_plot <- ggplot() +
  ...
  # Ensure predictions span the full range (1971-2006)
  scale_x_continuous(limits = c(1971, 2006), breaks = seq(1971, 2006, by = 5)) +
...
# Save the plot
saveRDS(dip_abund_plot, "Plots/Diptera/Diptera_abundance_model.rds")
ggsave("Plots/Diptera/Diptera_abundance_model.png", plot = dip_abund_plot, width = 8, height = 7, dpi = 300, bg = "white")
```

```{r}
#| label: Plot abundance (in thousands) models with back transformed variables
#| fig-height: 8
#| fig-width: 12
...
# Save the plot
saveRDS(dip_abund_plot, "Plots/Diptera/Diptera_abundance_model_thousand.rds")
ggsave("Plots/Diptera/Diptera_abundance_model_thousand.png", plot = dip_abund_plot, width = 8, height = 7, dpi = 300, bg = "white")
```
...
TD |>
  ...
  labs(y = "Diptera Abundance",
       ...
       colour = "Trap",
       fill = "Trap") +
  scale_x_continuous(limits = c(1971, 2006)) +
  ylim(range(TD$Abund)) +
  My_theme +
...
TD_red |>
  ...
  # geom_smooth(method = lm, formula = y ~ splines::ns(x, 1), se = TRUE, method.args = list(family = "nbinomial")) +
  # geom_smooth(method = lm, formula = y ~ x, se = TRUE, method.args = list(family = "nbinomial")) +
  geom_jitter(shape = 19, size = 3.5, height = 0.5,
              width = 0.5, alpha = 0.5) +
  labs(y = "Diptera Abundance",
       x = "Year",
...
## Check non-linearity of richness

```{r}
#| label: Check non-linearity in richness
#| fig-height: 8
#| fig-width: 12
...
spp_mod_overall5 \<- glmmTMB(SppRich \~ ns(year.std, df = 4) + (1 \| ftrap) + ar1(0 + factor(year.std) \| ftrap),
...
# smoother model with trap as a random effect
spp_mod_overall3 <- glmmTMB(SppRich ~ poly(year.std, 2) + (1 | ftrap), 
                        family = Gamma(link = "log"), 
                        data = TD_red)

# smoother model with trap as a random effect and AR(1) autocorrelation
spp_mod_overall4 <- glmmTMB(SppRich ~ poly(year.std, 2) + (1 | ftrap) + ar1(0 + factor(year.std) | ftrap),
                        family = Gamma(link = "log"), 
                        data = TD_red)

# spline model with trap as a random effect and AR(1) autocorrelation
spp_mod_overall5 <- glmmTMB(SppRich ~ ns(year.std, df = 4) + (1 | ftrap) + ar1(0 + factor(year.std) | ftrap),
                            family = Gamma(link = "log"), 
                            data = TD_red)

# Compare models
AIC(spp_mod_overall, spp_mod_overall1, spp_mod_overall2, spp_mod_overall3, spp_mod_overall4, spp_mod_overall5)
...
tab_model(
  ...
  file = "Plots/Diptera/Diptera_sppRich_overall_model.html"
)
...
tab_model(
  ...
  file = "Plots/Diptera/Diptera_sppRich_trap_models.html"
)
```
...
MyData_overall$ftrap <- factor("Overall", levels = c("Overall", levels(TD_red$ftrap)))
...
ggplot() +
  ...
  geom_line(data = MyData_overall, aes(x = year.std, y = predicted, linetype = ftrap), colour = "black", size = 2) +
  labs(
    title = "Diptera Richness Trends: Trap-Specific & Overall",
    x = "Standardized Year",
...
# Save the plot
saveRDS(dip_spprich_plot, "Plots/Diptera/Diptera_sppRich_model.rds")
ggsave("Plots/Diptera/Diptera_sppRich_model.png", plot = dip_spprich_plot, width = 8, height = 7, dpi = 300, bg = "white")
```
...
## Check non-linearity of turnover

```{r}
#| label: Check non-linearity in turnover
#| fig-height: 8
#| fig-width: 12
...
# Gamma Model (if data is continuous and positive)
mod_gaussian_logit <- glmmTMB(Turnover ~ year.std, 
                     family = gaussian(link = "logit"),
                     data = TD_turn_red)

# Gaussian model (if turnover is approximately normal)
mod_gaussian_logit <- glmmTMB(Turnover ~ year.std, 
                     family = gaussian(link = "logit"),
                     data = TD_turn_red)

# Compare models using AIC
AIC(mod_poisson, mod_gaussian_logit, mod_gaussian, mod_gam)
```
...
# spline model with trap as a random effect and AR(1) autocorrelation
turn_mod_overall5 <- glmmTMB(Turnover ~ ns(year.std, df = 4) + (1 | ftrap) + ar1(0 + factor(year.std) | ftrap),
                             family = gaussian(link = "logit"),
                             data = TD_turn_red)
...
tab_model(
  ...
  file = "Plots/Diptera/Diptera_turnover_overall_model.html"
)
...
tab_model(
  ...
  file = "Plots/Diptera/Diptera_turnover_trap_models.html"
)
```
...
MyData_traps$ftrap <- factor(MyData_traps$ftrap, levels = levels(TD$ftrap))
MyData_overall$ftrap <- factor("Overall", levels = c("Overall", levels(TD_turn_red$ftrap)))
...
ggplot() +
  ...
  geom_line(data = MyData_overall, aes(x = year.std, y = predicted, linetype = ftrap), colour = "black", size = 2) +
  labs(
    # title = "Diptera Turnover Trends: Trap-Specific & Overall",
    x = "Year",
    y = "Diptera Turnover",
    colour = "Trap",
    fill = "Trap",
...
# Save the plot
saveRDS(dip_turnover_plot, "Plots/Diptera/Diptera_turnover_model.rds")
ggsave("Plots/Diptera/Diptera_turnover_model.png", plot = dip_turnover_plot, width = 8, height = 7, dpi = 300, bg = "white")
```
