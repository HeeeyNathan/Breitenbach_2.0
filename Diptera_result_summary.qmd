...
mod_overall <- glmmTMB(Abund ~ bs(year.std), 
                        family = nbinom2, 
                        data = TD)

mod_overall1 <- glmmTMB(Abund ~ bs(year.std) + (1 | ftrap), 
                        family = nbinom2, 
                        data = TD)
...
mod_overall4 <- glmmTMB(Abund ~ poly(year.std, 3) + (1 | ftrap) + ar1(0 + factor(year.std) | ftrap), 
                        family = nbinom2, 
                        data = TD)
...
simulation_output <- simulateResiduals(mod_overall2)
plot(simulation_output)

# Check for temporal autocorrelation in residuals
acf(residuals(mod_overall2), main = "Autocorrelation of Residuals")
...
pred_overall <- predict(mod_overall2, newdata = MyData_overall, type = "response", se.fit = TRUE, allow.new.levels = TRUE)
...
# Save the plot
ggsave("Plots/Diptera/Diptera_abundance_model.png", width = 8, height = 7, dpi = 300, bg = "white")
```
...
# Compare models
AIC(spp_mod_overall, spp_mod_overall1, spp_mod_overall2, spp_mod_overall3)
...
# Simulated residual diagnostics
simulation_output <- simulateResiduals(spp_mod_overall2)
plot(simulation_output)

# Check for temporal autocorrelation in residuals
acf(residuals(spp_mod_overall2), main = "Autocorrelation of Residuals")
...
# Get predictions
pred_overall <- predict(spp_mod_overall2, 
...
ggplot() +
  ...
  labs(
    # title = "Diptera Richness Trends: Trap-Specific & Overall",
    x = "Year",
    y = "Diptera species richness (q = 0)",
    colour = "Trap",
    fill = "Trap",
    shape = "Trap",
    linetype = "Model"
...
# Save the plot
ggsave("Plots/Diptera/Diptera_sppRich_model.png", width = 8, height = 7, dpi = 300, bg = "white")
```
...
# Gamma Model (if data is continuous and positive)
mod_gaussian_logit <- glmmTMB(Turnover ~ bs(year.std), 
...
# Compare models using AIC
AIC(mod_poisson, mod_gaussian_logit, mod_gaussian)
```
...
# Simulated residual diagnostics
simulation_output <- simulateResiduals(turn_mod_overall2)
plot(simulation_output)

# Check for temporal autocorrelation in residuals
acf(residuals(turn_mod_overall2), main = "Autocorrelation of Residuals")
...
pred_overall <- predict(turn_mod_overall2, 
                        newdata = MyData_overall, 
                        type = "response", 
                        se.fit = TRUE, 
                        allow.new.levels = TRUE)
...
# Save the plot
ggsave("Plots/Diptera/Diptera_turnover_model.png", width = 8, height = 7, dpi = 300, bg = "white")
```
