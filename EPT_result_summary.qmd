...
## Mblm trends in EPT family abundances per trap

```{r}
ept_summary <- ept_long %>%
  group_by(trap, year, family) %>%
  summarise(total_abundance = sum(abundance), .groups = "drop")

trend_df <- ept_summary %>%
...
plot(simulation_output, quantreg = T)
...
# spline model with trap as a random effect and AR(1) autocorrelation
turn_mod_overall6 <- glmmTMB(Turnover ~ ns(year.std, df = 4) + (1 | ftrap) + ar1(0 + factor(year.std) | ftrap),
                             family = gaussian(link = "logit"),
                             data = TD_turn_red)

# Compare models
AIC(turn_mod_overall, turn_mod_overall1, turn_mod_overall2, turn_mod_overall3, turn_mod_overall4, turn_mod_overall5, turn_mod_overall6)
